...
- [x] iOS保存問題：ダウンロードがファイル/ドライブに保存される→写真アプリへ確実に保存できる仕様に変更（新タブ表示＋長押し保存方式）
- [ ] AI加工品質劣化：FIT WARSのプロンプトを元の高品質設定（被写体の特徴を捕えた全身チビキャラ）に修復
- [ ] AI加工品質劣化：免許メーカーのプロンプトを元の高品質設定（被写体の特徴を捕えた3Dアニメ風）に修復
- [x] 保存方法をWeb Share API（iOS/Androidネイティブ共有シート）+a.downloadフォールバックに復元（5/12 12:16時点仕様）
- [x] AI加工：OpenAI gpt-image-1を正しいFormData方式で再実装（高品質1536px・JPEG 95%維持・multipart/form-data方式）
- [x] AIキュー方式：複数人同時生成対応（同時処理数制限・待ち人数表示・エラーにならない）
- [x] AI加工中表示：FIT WARSページに「AI加工中...」と待ち人数を表示
- [x] AI加工中表示：免許メーカーページに「AI加工中...」と待ち人数を表示
- [x] 免許メーカープレビューの写真サイズ・位置を修正（完成データは変更しない）
